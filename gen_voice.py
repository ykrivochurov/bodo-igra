...
# слова кроссворда (капс-ключ, Тайтл в синтезе)
CW_WORDS = ["КОТ", "СОК", "ЗЕБРА", "АРБУЗ", "ЗОНТ", "МАШИНА", "ШАРИК", "НОСОК",
            "СОБАКА", "БАНАН", "КНИГА", "ЛИСА", "СПУТНИК", "РАКЕТА", "ЗВЕЗДА",
            "КОСМОС", "ЗЕМЛЯ", "ДЕРЕВО", "ОБЛАКО", "СОЛНЦЕ", "РАДУГА", "ДОЖДИК",
            "КАПУСТА", "ПОМИДОР", "ОГУРЕЦ", "ЛИМОН", "МОЛОКО", "ВЕЛОСИПЕД",
            "САМОЛЁТ", "КОРАБЛЬ", "ПОЕЗД"]
for w in CW_WORDS:
    add(w, w.title() + "!")
add("Картинка подсказывает, какое слово где начинается! Нажми на слово, а потом на его клеточки.")
